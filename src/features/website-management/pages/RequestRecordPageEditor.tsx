...
import { getRequestRecordsPage, updateRequestRecordsPage } from "@/api/endpoints/request-records.api";
...
export default function RequestRecordPageEditor() {
      ...
      try {
        ...
        if (pageResponse.data) {
          ...
          if (w1) {
            ...
            setWidget1Items(w1.items?.map(i => ({ id: i.id || Math.random().toString(), content: i.text })) || []);
          }
          if (w2) {
            setWidget2Title(w2.title || "");
            setWidget2Items(w2.items?.map(i => ({ id: i.id || Math.random().toString(), content: i.text })) || []);
          }
          if (w3) {
            setWidget3Title(w3.title || "");
            setWidget3Items(w3.items?.map(i => ({ id: i.id || Math.random().toString(), content: i.text })) || []);
          }
        }
      } catch (error) {
  ...
  const [widget3Items, setWidget3Items] = useState([
    { id: "1", content: "Contact our Privacy Officer for assistance with records requests." },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (widget1Title || widget1Items.length > 0) {
        widgets.push({ title: widget1Title, items: widget1Items.map(i => ({ text: i.content })) });
      }
      if (widget2Title || widget2Items.length > 0) {
        widgets.push({ title: widget2Title, items: widget2Items.map(i => ({ text: i.content })) });
      }
      if (widget3Title || widget3Items.length > 0) {
        widgets.push({ title: widget3Title, items: widget3Items.map(i => ({ text: i.content })) });
      }
  ...
  const handleAddWidget1Item = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && widget1NewItem.trim()) {
      e.preventDefault();
      setWidget1Items(prev => [...prev, { id: Math.random().toString(36).substring(7), content: widget1NewItem.trim() }]);
      setWidget1NewItem("");
      setIsDirty(true);
    }
  };
  ...
  const handleAddWidget2Item = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && widget2NewItem.trim()) {
      e.preventDefault();
      setWidget2Items(prev => [...prev, { id: Math.random().toString(36).substring(7), content: widget2NewItem.trim() }]);
      setWidget2NewItem("");
      setIsDirty(true);
    }
  };
  ...
  const handleAddWidget3Item = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && widget3NewItem.trim()) {
      e.preventDefault();
      setWidget3Items(prev => [...prev, { id: Math.random().toString(36).substring(7), content: widget3NewItem.trim() }]);
      setWidget3NewItem("");
      setIsDirty(true);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-7 w-full space-y-8 min-h-full font-sans pb-20 bg-slate-50">
      ...
      <div className="space-y-6">
        ...
        {/* Side Widget 1 */}
        <div className="border border-slate-200 rounded-xl p-6 bg-white shadow-sm">
          <h3 className="text-lg font-bold text-slate-800 mb-5">Side Widget 1</h3>
          <div className="space-y-6">
            <div>
              ...
              <input
                type="text"
                value={widget1Title}
                onChange={(e) => { setWidget1Title(e.target.value); setIsDirty(true); }}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-[#272628] mb-1.5">Items:</label>
                <input
                  type="text"
              ...
              <div className="space-y-3 pt-2">
                {widget1Items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setWidget1Items(prev => prev.filter(i => i.id !== item.id)); setIsDirty(true); }}
        ...
        {/* Side Widget 2 */}
        <div className="border border-slate-200 rounded-xl p-6 bg-white shadow-sm">
          <h3 className="text-lg font-bold text-slate-800 mb-5">Side Widget 2</h3>
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-[#272628] mb-1.5">Title:</label>
              <input
                type="text"
                value={widget2Title}
                onChange={(e) => { setWidget2Title(e.target.value); setIsDirty(true); }}
              ...
              <div className="space-y-3 pt-2">
                {widget2Items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setWidget2Items(prev => prev.filter(i => i.id !== item.id)); setIsDirty(true); }}
                      className="text-red-500 hover:text-red-700 hover:bg-red-50 w-8 h-8 flex items-center justify-center rounded-md transition-colors shrink-0"
                      title="Remove Item"
                    >
                      <Trash2 size={16} strokeWidth={2} />
          ...
          <div className="space-y-6">
            <div>
              ...
              <input
                type="text"
                value={widget3Title}
                onChange={(e) => { setWidget3Title(e.target.value); setIsDirty(true); }}
              ...
              <div className="space-y-3 pt-2">
                {widget3Items.map((item) => (
                  <div key={item.id} className="flex items-center gap-4">
                    ...
                    <button
                      type="button"
                      onClick={() => { setWidget3Items(prev => prev.filter(i => i.id !== item.id)); setIsDirty(true); }}
                      className="text-red-500 hover:text-red-700 hover:bg-red-50 w-8 h-8 flex items-center justify-center rounded-md transition-colors shrink-0"
                      title="Remove Item"
                    >
                      <Trash2 size={16} strokeWidth={2} />
