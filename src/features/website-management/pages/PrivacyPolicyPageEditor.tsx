...
export default function PrivacyPolicyPageEditor() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [heroes, widget, contentRes] = await Promise.all([
        getHeroSectionByPage("PrivacyPolicy").catch(() => []),
        getSideWidgetByPage("PrivacyPolicy").catch(() => []),
        getPolicyPageContent("privacy-policy").catch(() => null)
      ]);
  ...
  const handleSave = async (sectionName?: string) => {
    ...
    try {
      ...
      // Save Content
      if (!sectionName || sectionName === "Privacy Policy Content") {
        tasks.push(updatePolicyPageContent("privacy-policy", {
    ...
    <div className="p-4 sm:p-7 max-w-7xl mx-auto space-y-8 min-h-full font-sans pb-20 bg-slate-50">
      ...
      <div className="space-y-6">
        ...
        <PolicyContentSection
          ...
          onSave={() => handleSave("Privacy Policy Content")}
          isSaving={savingSection === "Privacy Policy Content"}
        />
