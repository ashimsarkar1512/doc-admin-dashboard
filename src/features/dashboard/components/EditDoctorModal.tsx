...
export default function EditDoctorModal({ isOpen, onClose, doctorId }: Props) {
  ...
  useEffect(() => {
    if (doctor && isOpen) {
      ...
      setUploadedAvatarId(null); // Reset until a new one is uploaded
    }
  }, [doctor, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateDoctor(doctorId, {
        ...(uploadedAvatarId ? { avatarId: uploadedAvatarId } : {}),
