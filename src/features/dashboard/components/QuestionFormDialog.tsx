...
// Local Option Type to handle inline creation/editing
type LocalOption = {
  id?: string;
  label: string;
  inputType?: string;
  placeholder?: string;
  _isDeleted?: boolean;
  subOptions?: { label: string; _isDeleted?: boolean }[];
};
...
export default function QuestionFormDialog({
  ...
  const handleAddSubOption = (index: number) => {
    const newOps = [...localOptions];
    if (!newOps[index].subOptions) {
      newOps[index].subOptions = [];
    }
    newOps[index].subOptions!.push({ label: '' });
    setLocalOptions(newOps);
  };

  const handleUpdateSubOption = (optIndex: number, subIndex: number, value: string) => {
    const newOps = [...localOptions];
    const subOps = [...(newOps[optIndex].subOptions || [])];
    subOps[subIndex] = { ...subOps[subIndex], label: value };
    newOps[optIndex].subOptions = subOps;
    setLocalOptions(newOps);
  };

  const handleRemoveSubOption = (optIndex: number, subIndex: number) => {
    const newOps = [...localOptions];
    const subOps = [...(newOps[optIndex].subOptions || [])];
    subOps[subIndex] = { ...subOps[subIndex], _isDeleted: true };
    newOps[optIndex].subOptions = subOps;
    setLocalOptions(newOps);
  };
                    ...
                    <div key={opt.originalIndex} className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="relative flex-1">
                          <input
                            className="w-full px-4 py-2.5 border border-gray-200 rounded-[10px] text-sm bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 pr-28"
                          ...
                          <button
                            type="button"
                            onClick={() => handleAddSubOption(opt.originalIndex)}
                      ...
                      {opt.subOptions?.map((subOpt, subIdx) => {
                        if (subOpt._isDeleted) return null;
                        return (
                          <div key={subIdx} className="flex items-center gap-3 pl-10">
                            <input
                              className="flex-1 px-4 py-2.5 border border-gray-200 rounded-[10px] text-sm bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20"
                              placeholder="Sub-option value"
                              value={subOpt.label}
                              onChange={e => handleUpdateSubOption(opt.originalIndex, subIdx, e.target.value)}
                            />
                            <button
                              type="button"
                              onClick={() => handleRemoveSubOption(opt.originalIndex, subIdx)}
                              className="text-red-500 hover:text-red-700 hover:bg-red-50 p-2 rounded-lg transition-colors flex-shrink-0"
                            >
                              <Trash2 className="w-[18px] h-[18px]" />
                            </button>
