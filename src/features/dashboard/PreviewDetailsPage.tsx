...
// Helper to render dynamic questions based on type
function RenderQuestion({ question }: { question: AssessmentDetails['questions'][0] }) {
  return (
    <SectionCard key={question.id}>
      {question.heading && <p className="text-xs text-slate-400 font-medium mb-1">{question.heading}</p>}
      ...
      <div className="space-y-2 mt-2">
        ...
        {/* Selected Options (Checkboxes/Radio - handles all cases) */}
        {question.patientAnswer.selectedOptions.length > 0 && (
          <div className="space-y-2">
            {question.patientAnswer.selectedOptions.map((opt, idx) => {
              // Try to find label from options, if not just use the value
              const originalOption = question.options.find(o => o.id === opt || o.label === opt);
              const displayLabel = originalOption?.label || opt;
              
              return question.type === 'RADIO' ? (
                <RadioAnswer key={idx} text={displayLabel} />
              ) : (
                <CheckboxAnswer key={idx} text={displayLabel} />
              );
            })}
          </div>
        )}
      ...
      <div className="space-y-4">
        ...
        <SectionCard>
          ...
          <div className="w-full h-44 rounded-xl overflow-hidden bg-slate-100">
            <img
              src={details.assessment.thumbnail}
        ...
        {details.doctorNotes && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-5">
            <p className="font-semibold text-slate-800 text-sm">Assessment Decline Reason</p>
            {details.reviewedBy && (
              <p className="text-xs text-slate-500 mt-1">Reviewed by: {details.reviewedBy.name}</p>
            )}
